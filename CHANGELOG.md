...
- Added `portalProps` prop to `Select`, `MultiSelect`, `Toast`, `AlertDialog`, `SimpleAlertDialog`, `ModalDialog`, and `SimpleModalDialog` components

### Updated

- Updated `Dropdown`, `Popover`, and `Tooltip` to fix issue with `portalProps` overriding the `className` prop
...
- Updated `Dropdown` component to accept a `ref` prop
- Merged `ref` prop with `setFloating` ref from `floating-ui`
- Updated `Dropdown` component to use `forwardRef` for consistency with other helium components
...
- Updated `home` icon to fix rendering issue
...
- Update `Dropdown`
  - Added `onClick` to `RenderOpenerProps`
- Update `Popover`
  - Added `onClick` and `onKeyPress` to `RenderOpenerProps`

## [2.5.2] - 2023-11-30

### Changed

- Update `DropdownMenuButton`
  - Updated `onClick`, `onEnter` and `onSelect` types interface to address `e` possibly undefined error
- Update `DropdownMenuLink`
  - Updated `onClick` type interface to address `e` possibly undefined error
...
- Update `DropdownMenuButton`
  - Added `e.preventDefault()` on button `onMouseDown` to prevent `onBlur` events on other elements from firing before the button's `onClick`
...
- Updated `DropdownMenuButton`
  - Update props extension to `ButtonHTMLAttributes`
  - Added optional event parameter to `onClick` prop
- Updated `DropdownMenuLink`
...
- Updated `Dropdown` component
  - Made `Portal` import a relative path
- Updated `Popover` component
  - Made `Portal` and `Button` imports relative paths
...
- Updated `Dropdown` component
  ...
  - Updated style property from `minWidth` to `width`

### Removed

- Remove `grow` prop from `Dropdown` component
...
- Added new `arrow-up-circle--filled` svg icon asset
- Added new `arrow-down-circle--filled` svg icon asset

### Changed

- Changed `TextInput` and `RadioButton` components to add ref forwarding
- Updated `Dropdown` component menu styling to add an outline that matches the new `Popover` styling
- Recompiled icon font
...
- Updated `fontcustom.yml` to auto autogenerate `icon.ts` icon type file
- Recompiled icon font
...
- Removed `dist/js/components/Input.d.ts`
- Removed `dist/js/components/Input.js`
- Removed `dist/js/components/Input.js.map`
...
- Added new `resize-se` svg icon asset
...
- Added new `question-mark-circle` svg icon asset

### Changed

- Renamed `question-mark-circle` svg icon assets to `question-mark-circle--filled`
- Recompiled icon font
...
- Added `h-link` utility class from `crm-web` bridge file
- Added `h-text-decoration-underline-hover` utility class from `crm-web` bridge file

### Changed

- Updated `useDialog` hook to handle `modal:close` event listeners when the `open` prop is updated

## [2.0.26] - 2023-06-30

### Added

- Added Docker sync tooling directly into this repo
